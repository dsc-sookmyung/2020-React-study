...
let myCat = new Cat("soo");
myCat.hello(); // Mew~ I'm soo
...
let myCheeseCat = new CheeseCat("sooo");

myCheeseCat.hello(); // Cheese~ I'm sooo
...
//Q4-2
let result_filter = users.filter((v) => {
    return v.age >= 28;
}).map((v) => {
    return `${v.name}, ${v.age}`
});
...
//Q4-3
let result_with_index = users.map((v, i) => {
    return `${i}. ${v.name}, ${v.age}`
});

console.log(result_with_index);


/*
04-1. ["Alice, 18", "Bob, 30", "Carol, 22", "Dave, 28"]

04-2. ["Bob, 30", "Dave, 28"]

04-3. ["0. Alice, 18", "1. Bob, 30", "2. Carol, 22", "3. Dave, 28"]
